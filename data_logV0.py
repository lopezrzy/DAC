...
def read_device(serial_wrapper, io, csv_writer):
    ...
    print(f"Data from {io}: {data}")
        
    humidity, temperature, current_time = parse_data(data)
    # Write data to the CSV file
    csv_writer.writerow([current_time, temperature, humidity, io])
...
with open(csv_filename, mode="w", newline="") as csv_file:
    ...
    csv_writer.writerow(["Time", "Temperature (C)", "RH (%)", "IO"])
    ...
    try:
        ...
        while True:
            ...
            for i, device_number in enumerate(device_numbers):
                read_device(THUM_devices[i], IO[i], csv_writer)
